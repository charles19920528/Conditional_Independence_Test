...
alternative_result_dict_100 = dict()
for j in np.arange(simulation_times):
    x_y_mat = ising_simulation.generate_x_y_mat()
    alternative_result_dict_100[j] = ising_simulation.trainning(x_y_mat, print_loss_boolean=False)
    print("Training finished. Sample size : %d, simulation sample: %d" % (sample_size, j))


with open("./results/alternative_result_dict_100.p", "wb") as fp:
    pickle.dump(alternative_result_dict_100, fp, protocol=pickle.HIGHEST_PROTOCOL)
