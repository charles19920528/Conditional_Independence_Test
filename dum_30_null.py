...
null_result_dict_30 = dict()
for j in np.arange(simulation_times):
    x_y_mat = ising_simulation.generate_x_y_mat()
    null_result_dict_30[j] = ising_simulation.trainning(x_y_mat, print_loss_boolean=False)
    print("Training finished. Sample size : %d, simulation sample: %d" % (sample_size, j))


with open("./results/null_result_dict_30.p", "wb") as fp:
    pickle.dump(null_result_dict_30, fp, protocol=pickle.HIGHEST_PROTOCOL)
