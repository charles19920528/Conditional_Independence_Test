...
def theta_derivatives_beta(x_y_mat, final_linear_input_mat, j_mat, gradient_boolean, hessian_boolean):
    if not (gradient_boolean or hessian_boolean):
        return None
    result_list = []
...
def wald_test(x_y_mat, z_mat, network_test_args_dict: dict, network_weights_vet):
    ...
    inverse_hessian_mat = np.linalg.solve(hessian_mat, np.identity(hessian_mat.shape[0]))
...
def wald_test_beta(x_y_mat, z_mat, network_test_args_dict: dict, network_weights_vet):
    network_test = gt.FullyConnectedNetworkTest(**network_test_args_dict)
    network_test.call(z_mat[:2, :])
    network_test.set_weights(network_weights_vet)
    theta_vet = np.concatenate([network_weights_vet[-2][:, 2], [network_weights_vet[-1][2]]]).reshape(-1, 1)
    ...
    gradient_mat, hessian_mat = theta_derivatives_beta(x_y_mat=x_y_mat, final_linear_input_mat=final_linear_input_mat,
                                                       j_mat=j_mat, gradient_boolean=True, hessian_boolean=True)
    # inverse_hessian_mat = np.linalg.solve(hessian_mat, np.identity(hessian_mat.shape[0]))
