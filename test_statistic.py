...
def theta_derivatives(x_y_mat, final_linear_input_mat, j_mat, gradient_boolean, hessian_boolean):
    ...
    if gradient_boolean:
        ...
        # n by p
        gradient_mat = final_linear_input_mat * multiplier_vet.reshape(-1, 1)
    ...
    if hessian_boolean:
        ...
        for i in np.arange(hessian_mat.shape[0]):
            for j in np.arange(i, hessian_mat.shape[0]):
                ...
                hessian_mat[i, j] = hessian_mat[j, i] = second_derivative_vet.sum()
...
class WaldTest:
    def __init__(self, x_y_mat, z_mat, network_test: gt.FullyConnectedNetworkTest, network_weights_vet):
        ...
        self.network_test.call(x_y_mat[0, :])
        self.network_test.set_weights(network_weights_vet)

        self.theta_vet = np.concatenate([network_weights_vet[-2][:, 2], [network_weights_vet[-1][2]]])

        self.j_mat = self.network_test.call(z_mat)
