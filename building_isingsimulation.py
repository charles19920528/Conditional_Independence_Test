...
class IsingSimulation:
    def __init__(self, z_mat, true_network, null_boolean, hidden_1_out_dim,learning_rate, buffer_size, batch_size,
        ...
        self.true_parameter = true_network(z_mat)
    ...
    def trainning(self, x_y_mat):
        ...
        :return: result_dict: A dictionary which contains two keys which are "loss_array" and "jxy".
        result_dict["loss_array"] is a 2 by epoch numpy of which the first row stores the (- 2 * LogLikelihood) and the
        second row stores the kl divergences. result_dict["jxy"] stores a scalar which is the fitted value of Jxy in
        the full Ising Model.
        """
